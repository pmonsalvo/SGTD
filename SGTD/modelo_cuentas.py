...
class Cuenta(object):
    ...
    def getIdCuentas(self):
        return self.__idPagos
    def setIdCuentas(self, valor):
...
def buscar(codigo):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('select * from Cuentas where idCuentas = %s', (codigo,))
...
def buscarPendientes(codigo):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('select * from Cuentas where idCuentas = %s and estado = %s', (codigo,'PENDIENTE',))
        filas = cursor.fetchall()
        cursor.close()
...
def crear(cuenta):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('insert into Cuentas (idfactura, idcliente, cuotas, monto, fecha_vencimiento, total, entrega, estado) values(%s, %s, %s, %s, %s, %s, %s, %s)',
                       (cuenta.getIdFactura(), cuenta.getIdCliente(), cuenta.getCuotas(), cuenta.getMonto(), cuenta.getFecha_Venc(), cuenta.getTotal(), cuenta.getEntrega(), cuenta.getEstado()))
        bd.get_db().commit()
        cursor.close()
        salida = True
...
def actualizar(cuenta):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('update Cuentas set idfactura = %s, idcliente = %s, cuotas = %s, monto = %s, fecha_vencimiento = %s, total = %s, entrega = %s, estado = %s where idCuentas = %s',
                       (cuenta.getIdFactura(), cuenta.getIdCliente(), cuenta.getCuotas(), cuenta.getMonto(), cuenta.getFecha_Venc(), cuenta.getTotal(), cuenta.getEntrega(), cuenta.getEstado(), cuenta.getIdCuentas()))
        bd.get_db().commit()
        cursor.close()
        salida = True
