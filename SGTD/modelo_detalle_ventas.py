...
class DetVenta(object):
    ...
    def getIdDetalle(self):
        return self.__idDetalle_Factura
    def setIdDetalle(self, valor):
        self.__idDetalle_Factura = valor
...
def buscar(codigo):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('select * from Detalle_Factura where idFactura = %s', (codigo,))
        filas = cursor.fetchall()
        cursor.close()
...
def crear(detVenta):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('insert into Detalle_Factura (idFactura, idProducto, cantidad, descripcion, precio_unitario, importe) values(%s, %s, %s, %s, %s, %s)',
                       (detVenta.getIdFactura(), detVenta.getIdProducto(), detVenta.getCantidad(), detVenta.getDescripcion(), detVenta.getPrecioU(), detVenta.getImporte()))
        bd.get_db().commit()
        cursor.close()
        salida = True
...
def eliminar(codigo):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('delete from Detalle_Factura where idFactura = %s', (codigo,))
        bd.get_db().commit()
        cursor.close()
        salida = True
...
def actualizar(detVenta):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('update Detalle_Factura set idProducto = %s, cantidad = %s, descripcion = %s, precio_unitario = %s, importe = %s where idFactura = %s',
                       (detVenta.getIdFactura(), detVenta.getCantidad(), detVenta.getDescripcion(), detVenta.getPrecioU(), detVenta.getImporte(), detVenta.getIdFactura()))
        bd.get_db().commit()
        cursor.close()
        salida = True
