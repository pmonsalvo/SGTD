...
# -*- coding: utf-8 -*-
"""
Modelo de la tabla proveedores con sus métodos de acceso a datos.
...
def buscar(codigo):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('select * from Proveedores where idProveedores = %s', (codigo,))
...
def crear(proveedor):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('insert into Proveedores (tipo_persona, nombre_razon, direccion, telefono, ruc_cedula, email) values(%s, %s, %s, %s, %s, %s)',
                       (proveedor.getTipo(), proveedor.getNombre(), proveedor.getDireccion(), proveedor.getTelefono(), proveedor.getRuc_Cedula(), proveedor.getEmail()))
        bd.get_db().commit()
        cursor.close()
        salida = True
...
def eliminar(codigo):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('delete from Proveedores where idProveedores = %s', (codigo,))
        bd.get_db().commit()
        cursor.close()
        salida = True
...
def actualizar(proveedor):
    ...
    if (bd.get_db() == None):
        ...
        cursor = bd.get_db().cursor()
        cursor.execute('update Proveedores set tipo_persona = %s, nombre_razon = %s, direccion = %s, telefono = %s, ruc_cedula = %s, email = %s where idProveedores = %s',
                       (proveedor.getTipo(), proveedor.getNombre(), proveedor.getDireccion(), proveedor.getTelefono(), proveedor.getRuc_Cedula(), proveedor.getEmail(), proveedor.getId()))
        bd.get_db().commit()
        cursor.close()
        salida = True
